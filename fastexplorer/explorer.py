...
def _get_numpy_bytes(x, typ, xtra=None):
...
#         if path not in self.cache['hook_layers']: self.cache['hook_layers'].append(path)
        ...
        if hm.ndim < 2:
            ...
            array_bytes = _get_numpy_bytes(hm, clientEvents.SEND_HEATMAP, xtra={'path':path})
            await websocket.send_bytes(array_bytes)
